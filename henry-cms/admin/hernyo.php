...
function HERNYO_DATA() {

	$hernyo_folder = HERNYO_FOLDER;
	$hernyo_version = '0.1.0';
	require_once $hernyo_folder . 'lib/parsedown.php';

	$_DATA = [];
	$_DATA['version'] = $hernyo_version;
	$_DATA['site'] = include $hernyo_folder . 'config.php';
	...
	$db_data = file( $hernyo_folder . $_DATA['site']['db'], FILE_IGNORE_NEW_LINES);
	$_DATA['pages']= array();
...
function HernyoCMS($HERNYO) {

	// Ugly and quick templating. Looking for a best way to implement this...
	$template = file_get_contents('theme/'.$HERNYO['site']['theme'].'/index.html');
...
if ( !empty($_GET['e']) && in_array($_GET['e'], $xml_stuff) ) {
	...
	$HERNYO = HERNYO_DATA();
		...
		$HERNYO['prev_page'] = dirname($_SERVER['PHP_SELF']);
		$HERNYO['next_page'] = dirname($_SERVER['PHP_SELF']);
		$HERNYO['is_404'] = true;
